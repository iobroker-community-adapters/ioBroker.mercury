...
const translate = function (w){ // PLEASE HELP WITH THE TRANSLATION
    const word = {
        'DateTime':         {ru: 'Дата/Время', en: ''},
        'Serial number':    {ru: 'Серийный номер', en: ''},
        'Production date':  {ru: 'Дата изготовления', en: ''},
        'Software version': {ru: 'Версия ПО', en: ''},
    ...
    if (word[w]){
        ...
        event.emit('info', 'Пожайлуста помогите с переводом - [ ' + '\'' + w + '\': {ru: \'' + 'ВАШ ПЕРЕВОД\'' + ', en: \'' + 'YOUR TRANSLATION\'}, ] PullRequest приветствуется, перевод нужно добавить в функцию translate - https://github.com/instalator/ioBroker.mercury/blob/6a99a51747b1cbf01661c3cf413714846e2c679d/lib/mercury.js#L6');
        return w;
    }
};
...
module.exports.template = {
    1: {
        conf:     {
            model:    {val: null, name: '', desc: translate('model')},
            name:     {val: '', desc: translate('name')},
            user:     {val: 1, desc: translate('user')},
            addr:     {val: null, desc: translate('address')},
            protocol: {val: null, desc: translate('protocol')}
        },
        info:     {},
        metering: {
            EnergyMonth: {
                Current: {
                    T1:    {val: null, unit: 'kWh', desc: translate('Энергия за текущий месяц по тарифу 1')},
                    T2:    {val: null, unit: 'kWh', desc: translate('Энергия за текущий месяц по тарифу 2')},
                    T3:    {val: null, unit: 'kWh', desc: translate('Энергия за текущий месяц по тарифу 3')},
                    T4:    {val: null, unit: 'kWh', desc: translate('Энергия за текущий месяц по тарифу 4')},
                    Total: {val: null, unit: 'kWh', desc: translate('Энергия за текущий месяц Итого')}
                }
            }
        }
    },
    2: {
        conf:     {
            ...
            addr:       {val: null, desc: translate('address')},
            user:       {val: 1, desc: translate('user')},
            pwd:        {val: null, desc: translate('password')},
            protocol:   {val: null, desc: translate('protocol')},
            isExtended: {val: false, desc: translate('extended')}
        },
        info:     {},
        metering: {
            EnergyMonth: {
...
const f = {
    ...
    powerPhase:       function (device, index, cmd, response){
        event.emit('debug', 'powerPhase - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    cosf:             function (device, index, cmd, response){
        ...
        try {
            if (response.length === 15){
                ...
                event.emit('debug', 'cosf - t1: ' + one + ' t2: ' + two + ' t3: ' + three + ' t4: ' + four);
    ...
    protocol: {
        1: {
            readinfo: [ //чтение инфы о счетчике
                {code: 0x2f, cmd: [], desc: translate('Чтение серийного номера'), func: ff.readSN},
                {code: 0x28, cmd: [], desc: translate('Чтение версии и даты ПО'), func: ff.identCounter},
                {code: 0x2d, cmd: [], desc: translate('Чтение функции выходного оптрона'), func: ff.optronFunc},
                {code: 0x65, cmd: [], desc: translate('Чтение слова исполнения'), func: ff.variantExecution},
                {code: 0x66, cmd: [], desc: translate('Чтение даты изготовления'), func: ff.dateProd}
            ],
            first:    [// параметры читаемые один раз при запуске драйвера
                {code: 0x29, cmd: [], desc: translate('Чтение напряжения на литиевой батарее'), func: ff.batteryVolt}
            ],
            poll:     [
                {code: 0x27, cmd: [], desc: translate('Чтение содержимого тарифных аккумуляторов активной энергии'), func: ff.totalEnergy},
                {code: 0x2b, cmd: [], desc: translate('Чтение времени последнего отключения напряжения'), func: ff.voltageOff},
                {code: 0x2c, cmd: [], desc: translate('Чтение времени последнего включения напряжения'), func: ff.voltageOn},
                {code: 0x21, cmd: [], desc: translate('Чтение внутренних часов и календаря счетчика'), func: ff.readDateTime},
                {code: 0x22, cmd: [], desc: translate('Чтение лимита мощности'), func: ff.limitPower},
                {code: 0x23, cmd: [], desc: translate('Чтение лимита энергии за месяц'), func: ff.limitEnergyMonth},
                //{code: 0x32, cmd: currentMonth(), desc: translate('Чтение энергии за текущий месяц'), func: ff.month12Energy}
                {code: 0x32, cmd: [0x0f], desc: translate('Чтение энергии за текущий месяц'), func: ff.month12Energy}
                //{code: 0x69, cmd: [], desc: translate('Чтение времени наработки'), func: ff.runningTime},
                //{code: 0x37, cmd: [0x01, 0x07, 0x12, 0x29], desc: translate('Чтение получасовых мощностей или суточных срезов'), func: ff.readDay}, //ADDR-CMD-N48-dd-mon-yy-CRC
                //{code: 0x33, cmd: [0x00], desc: translate('Чтение максимумов U'), func: ff.readMaxU},
                //{code: 0x33, cmd: [0x01], desc: translate('Чтение максимумов I'), func: ff.readMaxU},
                //{code: 0x33, cmd: [0x02], desc: translate('Чтение максимумов P'), func: ff.readMaxU}
            ],
            fastpoll: [
                {code: 0x63, cmd: [], desc: translate('Чтение значений U,I,P'), func: ff.readUIP}
                //{code: 0x26, cmd: [], desc: translate('Чтение текущей мощности в нагрузке'), func: ff.currentPower}
            ],
            read:     [
                {cmd: 0x20, desc: translate('Чтение группового адреса счетчика')},
                {cmd: 0x24, desc: translate('Чтение флага сезонного времени ')},
                {cmd: 0x25, desc: translate('Чтение величины коррекции времени ')},
                {cmd: 0x27, desc: translate('Чтение содержимого тарифных аккумуляторов активной энергии')},
                {cmd: 0x2A, desc: translate('Чтение режима индикации')},
                {cmd: 0x2E, desc: translate('Чтение кол-ва действующих тарифов')},
                {cmd: 0x30, desc: translate('Чтение таблицы праздничных дней')},
                {cmd: 0x31, desc: translate('Чтение таблицы переключений тарифных зон ')},
                {cmd: 0x33, desc: translate('Чтение максимумов')},
                {cmd: 0x34, desc: translate('Чтение буфера событий вкл/выкл')},
                {cmd: 0x35, desc: translate('Чтение буфера событий отк/закр')},
                {cmd: 0x36, desc: translate('Чтение буфера событий параметризации')},
                {cmd: 0x37, desc: translate('Чтение получасовых мощностей или суточных срезов')},
                {cmd: 0x38, desc: translate('Чтение месячных срезов реактивной энергии')},
                {cmd: 0x39, desc: translate('Чтение буфера событий качества электричества')},
                {cmd: 0x3A, desc: translate('Чтение буфера событий реле')},
                {cmd: 0x60, desc: translate('Чтение тарифа')},
                {cmd: 0x61, desc: translate('Чтение времени по-следнего вскрытия крышки счётчика')},
                {cmd: 0x62, desc: translate('Чтение времени по-следнего закрытия крышки счётчика')},
                {cmd: 0x64, desc: translate('Чтение коэффициента коррекции хода часов (для чтения коэффициента коррекции без перемычки)')},
                {cmd: 0x67, desc: translate('Чтение времени индикации')},
                {cmd: 0x68, desc: translate('Чтение режима ли-мита мощности')},
                {cmd: 0x69, desc: translate('Чтение времени наработки')},
                {cmd: 0x6A, desc: translate('Чтение режима доп. индикации')},
                {cmd: 0x6B, desc: translate('Чтение времени последней парам. счётчика')},
                {cmd: 0x6C, desc: translate('Чтение номера мо-дема и системы и уровня сигнала')},
                {cmd: 0x6D, desc: translate('Чтение режима управления реле')},
                {cmd: 0x6E, desc: translate('Чтение потарифных лимитов Энергии (остатки)')},
                {cmd: 0x6F, desc: translate('Чтение флага разрешения индикации под батарейкой')},
                {cmd: 0x80, desc: translate('Чтение флага разрешения мо-дема PLC2')},
                {cmd: 0x81, desc: translate('Чтение доп. параметров сети (частота)  и теку-щего тарифа')},
                {cmd: 0x82, desc: translate('Чтение множителя таймаута')},
                {cmd: 0x85, desc: translate('Чтение содержимо-го тарифных акку-муляторов реактив-ной энергии')},
                {cmd: 0x86, desc: translate('Чтение параметра')}
            ],
            write:    [
                {cmd: 0x00, desc: translate('Установка нового сетевого адреса счетчика')},
                {cmd: 0x01, desc: translate('Установка нового группового адреса счетчика')},
                {cmd: 0x02, desc: translate('Установка внутренних часов и календаря счетчика')},
                {cmd: 0x03, desc: translate('Установка лимита мощности')},
                {cmd: 0x04, desc: translate('Установка лимита энергии за месяц')},
                {cmd: 0x05, desc: translate('Установка флага сезонного времени ')},
                {cmd: 0x06, desc: translate('Установка величины коррекции времени')},
                {cmd: 0x07, desc: translate('Установка функции выходного оптрона')},
                {cmd: 0x08, desc: translate('Установка скорости обмена ')},
                {cmd: 0x09, desc: translate('Установка режимаиндикации')},
                {cmd: 0x0A, desc: translate('Установка числа действующих тарифов')},
                {cmd: 0x0B, desc: translate('Установка тарифа')},
                {cmd: 0x0C, desc: translate('Сброс защёлки «напряжение батареи»')},
                {cmd: 0x0D, desc: translate('Установка времени индикации')},
                {cmd: 0x0F, desc: translate('Установка режима лимита мощности')},
                {cmd: 0x10, desc: translate('Установка таблицы праздничных дней')},
                {cmd: 0x11, desc: translate('Установка таблицы переключений тарифных зон ')},
                {cmd: 0x12, desc: translate('Сброс максимумов')},
                {cmd: 0x13, desc: translate('Сброс максимумов. Под перемычкой')},
                {cmd: 0x14, desc: translate('Сброс наработки батареи.')},
                {cmd: 0x20, desc: translate('Чтение группового адреса счетчика')},
                {cmd: 0x21, desc: translate('Чтение внутренних часов и календаря счетчика')},
                {cmd: 0x22, desc: translate('Чтение лимита мощности')},
                {cmd: 0x23, desc: translate('Чтение лимита энергии за месяц')},
                {cmd: 0x24, desc: translate('Чтение флага сезонного времени ')},
                {cmd: 0x25, desc: translate('Чтение величины коррекции времени ')},
                {cmd: 0x26, desc: translate('Чтение текущей мощности в нагрузке')},
                {cmd: 0x27, desc: translate('Чтение содержимого тарифных аккумуляторов активной энергии')},
                {cmd: 0x28, desc: translate('Чтение идентификационных данных счетчика')},
                {cmd: 0x29, desc: translate('Чтение напряжения на литиевой батарее')},
                {cmd: 0x2A, desc: translate('Чтение режима индикации')},
                {cmd: 0x2B, desc: translate('Чтение времени последнего отключения напряжения')},
                {cmd: 0x2C, desc: translate('Чтение времени последнего включения напряжения')},
                {cmd: 0x2D, desc: translate('Чтение функции выходного оптрона')},
                {cmd: 0x2E, desc: translate('Чтение кол-ва действующих тарифов')},
                {cmd: 0x2F, desc: translate('Чтение серийного номера')},
                {cmd: 0x30, desc: translate('Чтение таблицы праздничных дней')},
                {cmd: 0x31, desc: translate('Чтение таблицы переключений тарифных зон ')},
                {cmd: 0x32, desc: translate('Чтение месячных срезов ')},
                {cmd: 0x33, desc: translate('Чтение максимумов')},
                {cmd: 0x34, desc: translate('Чтение буфера событий вкл/выкл')},
                {cmd: 0x35, desc: translate('Чтение буфера событий отк/закр')},
                {cmd: 0x36, desc: translate('Чтение буфера событий параметризации')},
                ...
                {cmd: 0x61, desc: translate('Чтение времени по-следнего вскрытия крышки счётчика')},
                {cmd: 0x62, desc: translate('Чтение времени по-следнего закрытия крышки счётчика')},
                {cmd: 0x63, desc: translate('Чтение  значений U,I,P')},
                {
                    cmd:  0x64,
                    desc: translate('Чтение коэффициента  коррекции хода часовВведена для чтения коэффициента  коррекции без перемычки')
                },
                {cmd: 0x65, desc: translate('Чтение слова исполнения')},
                {cmd: 0x66, desc: translate('Чтение даты изготовления')},
                {cmd: 0x67, desc: translate('Чтение времени индикации')},
                {cmd: 0x68, desc: translate('Чтение режима ли-мита мощности')},
                {cmd: 0x69, desc: translate('Чтение времени наработки')},
                {cmd: 0x6A, desc: translate('Чтение режима доп. индикации')},
                {cmd: 0x6B, desc: translate('Чтение времени по-следней парам. счётчика')},
                {cmd: 0x6C, desc: translate('Чтение номера мо-дема и системы и уровня сигнала')},
                {cmd: 0x6D, desc: translate('Чтение режима управления реле')},
                {cmd: 0x6E, desc: translate('Чтение потариф-ных лимитов Энергии (остатки)')},
                {cmd: 0x6F, desc: translate('Чтение флага раз-решения индикации под батарейкой')},
                {cmd: 0x70, desc: translate('Установка режима доп. индикации')},
                {cmd: 0x71, desc: translate('Установка режима управления реле')},
                {cmd: 0x72, desc: translate('Установка пота-рифных лимитов энергии')},
                {cmd: 0x73, desc: translate('Установкафлага разрешения инди-кации под батарей-кой')},
                {cmd: 0x74, desc: translate('Установка флага разрешения работы с модемом PLC2')},
                {cmd: 0x75, desc: translate('Установка множи-теля таймаута')},
                {cmd: 0x78, desc: translate('Установка тариф-ного расписания сжатым методом')},
                {cmd: 0x79, desc: translate('Обмен данными с PLC1 модемом')},
                {cmd: 0x7A, desc: translate('Запись параметра')},
                {cmd: 0x80, desc: translate('Чтение флага разрешения мо-дема PLC2')},
                {cmd: 0x81, desc: translate('Чтение доп. па-раметров сети (частота)  и теку-щего тарифа')},
                {cmd: 0x82, desc: translate('Чтение множителя таймаута')},
                {cmd: 0x85, desc: translate('Чтение содержимо-го тарифных акку-муляторов реактив-ной энергии')},
                {cmd: 0x86, desc: translate('Чтение параметра')}
            ]
        ...
        2: {
            ...
            poll:     [
                {code: 0x05, cmd: [0x00, 0x00], desc: translate('Чтение накопленной энергии от сброса'), func: f.totalEnergy},
                {code: 0x05, cmd: [0x10, 0x00], desc: translate('Чтение накопленной энергии за текущий год'), func: f.yearEnergy},
                {code: 0x05, cmd: [0x40, 0x00], desc: translate('Чтение энергии за текущие сутки'), func: f.currentDayEnergy},
                {code: 0x05, cmd: [0x00, 0x01], desc: translate('Чтение энергия по 1 тарифу'), func: f.oneTarrif},
                {code: 0x05, cmd: [0x00, 0x02], desc: translate('Чтение энергия по 2 тарифу'), func: f.twoTarrif},
                {code: 0x05, cmd: [0x00, 0x03], desc: translate('Чтение энергия по 3 тарифу'), func: f.threeTarrif},
                {code: 0x05, cmd: [0x00, 0x04], desc: translate('Чтение энергия по 4 тарифу'), func: f.fourTarrif},
                {code: 0x04, cmd: [0x00], desc: translate('Чтение текущего времени'), func: f.readCurrentTime},
                {code: 0x04, cmd: [0x14, 0x00], desc: translate('Чтение байт состояния'), func: f.statusByte}
            ],
            fastpoll: [
                {code: 0x08, cmd: [0x16, 0x00], desc: translate('Мощность по фазам'), func: f.powerPhase},
                {code: 0x08, cmd: [0x16, 0x11], desc: translate('Напряжение по фазам'), func: f.voltagePhase},
                {code: 0x08, cmd: [0x16, 0x21], desc: translate('Ток по фазам'), func: f.currentPhase},
                {code: 0x08, cmd: [0x16, 0x30], desc: translate('Коэффициент мощности по фазам'), func: f.cosf}
            ],
            read:     [
                {cmd: 0x01, desc: translate('Ускоренный режим чтения индивидуальных параметров прибора'), func: f.acceleratedMode},
                {cmd: 0x04, desc: translate('Чтение множителя таймаута дополнительного интерфейса'), func: f.timeoutInterface2},
                {cmd: 0x06, desc: translate('Чтение режимов индикации'), func: f.indicationMode},
                {cmd: 0x07, desc: translate('Чтение значений времен перехода на летнее и зимнее время'), func: f.timeSummerWinter},
                {
                    cmd:  0x08,
                    desc: translate('Чтение времени контроля за превышением лимита мощностии параметров автовключения реле'),
                    func: f.timeLimitPRele
                },
                {cmd: 0x09, desc: translate('Чтение программируемых флагов'), func: f.programFlag},
                {
                    cmd:  0x0C, desc: translate('Чтение расписания утренних и вечерних максимумов мощности'),
                    func: f.timeMorningEveningMax
                },
                {
                    cmd:  0x0D,
                    desc: translate('Чтение значений утренних и вечерних максимумов мощности'),
                    func: f.valueMorningEveningMax
                },
                {
                    cmd:  0x11,
                    desc: translate('Чтение вспомогательных параметров: мгновенной активной, реактивной, полной мощности, фазных и линейных напряжений, тока, коэффициента мощности, частоты и небаланса'),
                    func: f.moreParam
                },

                {
                    cmd:  0x13,
                    desc: translate('Чтение параметров последней записи основного массива средних мощностей'),
                    func: f.lastArrPower
                },
                {cmd: 0x14, desc: translate('Чтение зафиксированных данных'), func: f.recordedData},
                {
                    cmd:  0x15,
                    desc: translate('Чтение параметров последней записи дополнительного массива средних мощностей'),
                    func: f.lastMoreArrPower
                }, {
                    cmd:  0x16,
                    desc: translate('Чтение вспомогательных параметров: мгновенной активной, реактивной, полной мощности, фазных и линейных, напряжений, тока, коэффициента мощности, частоты, небаланса'),
                    func: f.moreParam2
                },
                {cmd: 0x17, desc: translate('Чтение байта состояния тарификатора'), func: f.statusByteTariff},
                {cmd: 0x18, desc: translate('Чтение слова состояния управления нагрузкой'), func: f.statusLoadManagement},
                {cmd: 0x19, desc: translate('Чтение лимита мощности'), func: f.limitPower},
                {cmd: 0x1A, desc: translate('Чтение лимита энергии по тарифу 1–4'), func: f.limitEnergyTariff},
                {
                    cmd:  0x1B,
                    desc: translate('Чтение параметров индикации счетчика (по индицируемым тарифам)'),
                    func: f.paramIndicatorTariff
                },
                {
                    cmd:  0x1C,
                    desc: translate('Чтение параметров индикации счетчика (по периодам индикации)'),
                    func: f.paramIndicatorPeriod
                },
                {cmd: 0x1D, desc: translate('Чтение множителя таймаута основного интерфейса'), func: f.factorTimeout},
                {cmd: 0x1E, desc: translate('Чтение параметров режима учета технических потерь'), func: f.paramLossMode},
                {cmd: 0x1F, desc: translate('Чтение мощностей технических потерь'), func: f.powerTechLosses},
                {cmd: 0x20, desc: translate('Чтение допустимых значений'), func: f.validValues},
                {cmd: 0x21, desc: translate('Чтение значений времен усреднения'), func: f.valueAvgTime},
                {cmd: 0x22, desc: translate('Чтение тарифного расписания'), func: f.tariffSchedule},
                {cmd: 0x23, desc: translate('Чтение расписания праздничных дней'), func: f.holidaySchedules},
                {cmd: 0x24, desc: translate('Чтение состояния длительных операций'), func: f.statesLongOp},
                {cmd: 0x27, desc: translate('Чтение параметров PLC1'), func: f.paramPlc1},
                {cmd: 0x2A, desc: translate('Чтение серийного номера выносного дисплея из прибора учета'), func: f.snExtDisplay},
                {cmd: 0x00, desc: translate('Инициализация основного массива средних мощностей (срезов)')},
                {cmd: 0x01, desc: translate('Запись параметров индикации счетчика (по индицируемым тарифам)')},
                {cmd: 0x02, desc: translate('Запись параметров индикации счетчика (по периодам индикации)')},
                {cmd: 0x03, desc: translate('Запись параметров индикации счетчика')},
                {cmd: 0x04, desc: translate('Вкл./выкл. режима «Тест»')},
                {cmd: 0x05, desc: translate('Запись нового сетевого адреса счетчика')},
                {cmd: 0x06, desc: translate('Инициализация дополнительного массива средних мощностей (срезов)')},
                {cmd: 0x08, desc: translate('Фиксация данных')},
                {cmd: 0x0C, desc: translate('Установка времени')},
                {cmd: 0x0D, desc: translate('Коррекция времени в пределах ±4 мин. один раз в сутки')},
                {cmd: 0x10, desc: translate('Запрет записи параметров по PLC1')},
                {cmd: 0x11, desc: translate('Запись параметров PLC1')},
                {cmd: 0x14, desc: translate('Изменить параметры связи дополнительного интерфейса')},
                {cmd: 0x15, desc: translate('Изменить параметры связи основного интерфейса')},
                {cmd: 0x16, desc: translate('Перезапустить счетчик')},
                {cmd: 0x18, desc: translate('Разрешить/запретить автоматический переход на зимнее/летнее время')},
                {cmd: 0x19, desc: translate('Значения времени перехода для летнего и зимнего времени')},
                {cmd: 0x1A, desc: translate('Запись серийного номера выносного дисплея')},
                {cmd: 0x1B, desc: translate('Записать коэффициенты транс-формации Кн и Кт')},
                {cmd: 0x1D, desc: translate('Записать тарифное расписание')},
                {cmd: 0x1E, desc: translate('Записать расписание празднич-ных дней')},
                {cmd: 0x1F, desc: translate('Изменить пароль')},
                {cmd: 0x21, desc: translate('Инициализация регистров энергии')},
                {cmd: 0x22, desc: translate('Запись местоположения прибора')},
                {cmd: 0x23, desc: translate('Запись расписания утреннего и вечернего максимумов')},
                {cmd: 0x24, desc: translate('Сброс значений массива помесячных максимумов')},
                {
                    cmd:  0x26,
                    desc: translate('Установка времени контроля за превышением лимита мощностии параметров автовключения реле')
                },
                {cmd: 0x27, desc: translate('Изменение постоянной счетчика')},
                {cmd: 0x2A, desc: translate('Изменение режима тарификатора')},
                {cmd: 0x2C, desc: translate('Установка лимита активной мощности')},
                {cmd: 0x2D, desc: translate('Включение контроля превышения лимита активной мощности')},
                {cmd: 0x2E, desc: translate('Установка лимита потребленной активной энергии')},
                {cmd: 0x2F, desc: translate('Включение контроля превышения потребленной активной энергии')},
                {cmd: 0x30, desc: translate('Изменение режима импульсного выхода')},
                {cmd: 0x31, desc: translate('Изменение режима управления нагрузкой')},
                {cmd: 0x32, desc: translate('Изменение множителя таймаута основного интерфейса')},
                {cmd: 0x33, desc: translate('Изменение режима учета технических потерь')},
                {cmd: 0x34, desc: translate('Установка значений мощностей технических потерь')},
                {cmd: 0x35, desc: translate('Изменение режима светодиодного индикатора и импульсного выхода R+ по виду энергии')},
                {cmd: 0x36, desc: translate('Установка допустимых значений при контроле ПКЭ')},
                {cmd: 0x37, desc: translate('Установка времени усреднения значений напряжения и частоты')},
                {cmd: 0x00, desc: translate('Чтение текущего времени')},
                {cmd: 0x01, desc: translate('Чтение времени включения/выключения прибора')},
                {cmd: 0x02, desc: translate('Чтение времени коррекции часов прибора')},
                {cmd: 0x03, desc: translate('Чтение времени включения/выключения фазы 1 прибора')},
                {cmd: 0x04, desc: translate('Чтение времени включения/выключения фазы 2 прибора')},
                {cmd: 0x05, desc: translate('Чтение времени включения/выключения фазы 3 прибора')},
                {cmd: 0x06, desc: translate('Чтение времени начала/окончания превышения лимита мощности прибора')},
                {cmd: 0x07, desc: translate('Чтение времени коррекции тарифного расписания')},
                {cmd: 0x08, desc: translate('Чтение времени коррекции расписания праздничных дней')},
                {cmd: 0x09, desc: translate('Чтение времени сброса регистров накопленной энергии')},
                {cmd: 0x0A, desc: translate('Чтение времени инициализации массива средних мощностей')},
                {cmd: 0x0B, desc: translate('Чтение времени превышения лимита энергии по тарифу 1')},
                {cmd: 0x0C, desc: translate('Чтение времени превышения лимита энергии по тарифу 2')},
                {cmd: 0x0D, desc: translate('Чтение времени превышения лимита энергии по тарифу 3')},
                {cmd: 0x0E, desc: translate('Чтение времени превышения лимита энергии по тарифу 4')},
                {cmd: 0x0F, desc: translate('Чтение времени коррекции параметров контроля за превышением лимита мощности')},
                {cmd: 0x10, desc: translate('Чтение времени коррекции параметров контроля за превышением лимита энергии')},
                {cmd: 0x11, desc: translate('Чтение времени коррекции параметров учета технических потерь')},
                {cmd: 0x12, desc: translate('Чтение времени вскрытия/закрытия прибора')},
                {cmd: 0x13, desc: translate('Чтение времени и кода перепрограммирования прибора')},
                {cmd: 0x14, desc: translate('Чтение времени и кода слова состояния прибора')},
                {cmd: 0x15, desc: translate('Чтение времени коррекции расписания утренних и вечерних максимумов мощности')},
                {cmd: 0x16, desc: translate('Чтение времени сброса массива значений максимумов мощности')},
                {cmd: 0x17, desc: translate('Чтение времени включения/выключения тока фазы 1 прибора')},
                {cmd: 0x18, desc: translate('Чтение времени включения/выключения тока фазы 2 прибора')},
                {cmd: 0x19, desc: translate('Чтение времени включения/выключения тока фазы 3 прибора')},
                {cmd: 0x1A, desc: translate('Чтение времени начала/окончания магнитного воздействия')},
                {
                    cmd:  0x20,
                    desc: translate('Чтение времени выхода/возврата за мин.предельно допустимое значение напряжения в фазе 1')
                },
                {
                    cmd:  0x21,
                    desc: translate('Чтение времени выхода/возврата за мин.нормально допустимое значение напряжения в фазе 1')
                },
                {
                    cmd:  0x22,
                    desc: translate('Чтение времени выхода/возврата за макс. нормально допустимое значение напряжения в фазе 1')
                },
                {
                    cmd:  0x23,
                    desc: translate('Чтение времени выхода/возврата за макс. предельно допустимое значение напряжения в фазе 1')
                },
                {
                    cmd:  0x24,
                    desc: translate('Чтение времени выхода/возврата за мин.предельно допустимое значение напряжения в фазе 2')
                },
                {
                    cmd:  0x25,
                    desc: translate('Чтение времени выхода/возврата за мин.нормально допустимое значение напряжения в фазе 2')
                },
                {
                    cmd:  0x26,
                    desc: translate('Чтение времени выхода/возврата за макс. нормально допустимое значение напряжения в фазе 2')
                },
                {
                    cmd:  0x27,
                    desc: translate('Чтение времени выхода/возврата за макс. предельно допустимое значение напряжения в фазе 2')
                },
                {
                    cmd:  0x28,
                    desc: translate('Чтение времени выхода/возврата за мин.предельно допустимое значе-ние напряжения в фазе 3')
                },
                {
                    cmd:  0x29,
                    desc: translate('Чтение времени выхода/возврата за мин.нормально допустимое значе-ние напряжения в фазе 3')
                },
                {
                    cmd:  0x2A,
                    desc: translate('Чтение времени выхода/возврата за макс. нормально допустимое значе-ние напряжения в фазе 3')
                },
                {
                    cmd:  0x2B,
                    desc: translate('Чтение времени выхода/возврата за макс. предельно допустимое значе-ние напряжения в фазе 3')
                },
                {cmd: 0x2C, desc: translate('Чтение времени выхода/возврата за мин.предельно допустимое значе-ние частоты сети')},
                {cmd: 0x2D, desc: translate('Чтение времени выхода/возврата за мин.нормально допустимое значе-ние частоты сети')},
                {
                    cmd:  0x2E,
                    desc: translate('Чтение времени выхода/возврата за макс. нормально допустимое значе-ние частоты сети')
                },
                {
                    cmd:  0x2F,
                    desc: translate('Чтение времени выхода/возврата за макс. предельно допустимое значе-ние частоты сети')
                },
                {cmd: 0x30, desc: translate('Чтение параметров провалов/перенапряжений по фазе 1')},
                {cmd: 0x31, desc: translate('Чтение параметров провалов/перенапряжений по фазе 2')},
                {cmd: 0x32, desc: translate('Чтение параметров провалов/перенапряжений по фазе 3')},
                {cmd: 0x80, desc: translate('Время возникновения/пропадания обратного тока по фазе 1')},
                {cmd: 0x81, desc: translate('Время возникновения/пропадания обратного тока по фазе 2')},
                {cmd: 0x82, desc: translate('Время возникновения/пропадания обратного тока по фазе 3')},
                {cmd: 0x83, desc: translate('Время начала/окончания отсутствия напряжения при наличии тока по фазе 1')},
                {cmd: 0x84, desc: translate('Время начала/окончания отсутствия напряжения при наличии тока по фазе 2')},
                {cmd: 0x85, desc: translate('Время начала/окончания отсутствия напряжения при наличии тока по фазе 3')},
                {cmd: 0x86, desc: translate('Время начала/окончания нарушения чередования фаз')}
            ],
            readinfo: [
                {
                    code: 0x08,
                    cmd:  [0x00],
                    desc: translate('Чтение серийногономера счетчика и даты выпуска'),
                    func: f.readSnAndDate
                },
                {code: 0x08, cmd: [0x03], desc: translate('Чтение версии ПО'), func: f.firmwareVersion},
                {code: 0x08, cmd: [0x0B], desc: translate('Чтение местоположения прибора'), func: f.readLocation},
                {code: 0x08, cmd: [0x12], desc: translate('Чтение варианта исполнения'), func: f.variantExecution},
                {code: 0x08, cmd: [0x26], desc: translate('Чтение CRC16 ПО прибора'), func: f.crcDevice},
                {code: 0x08, cmd: [0x02], desc: translate('Чтение коэффициента трансформации'), func: f.transformRatio}
            ],
            write:    [
                /*cmd:				0x00, level: '2,3', 	desc: translate('Инициализация основного массива средних мощностей (срезов)'),
                cmd:				0x01, level: '1,2,3',	desc: translate('Запись параметров индикации счетчика (по индицируемым тарифам)'),
                cmd:				0x02, level: '1,2,3',	desc: translate('Запись параметров индикации счетчика (по периодам индикации)'),
                cmd:				0x03, level: '1,2,3',	desc: translate('Запись параметров индикации счетчика'),
                setTest:			0x04, level: '2,3',		desc: translate('Вкл./выкл. режима «Тест»'),
                setAddr:			0x05, level: '1,2,3',	desc: translate('Запись нового сетевого адреса счетчика'),
                cmd:				0x06, level: '2,3',		desc: translate('Инициализация дополнительного массива средних мощностей (срезов)'),
                Save:				0x08, level: '0',		desc: translate('Фиксация данных'),
                setTimeDate:		0x0C, level: '2,3',		desc: translate('Установка времени'),
                cmd:				0x0D, level: '1,2,3',	desc: translate('Коррекция времени в пределах ±4 мин. один раз в сутки'),
                cmd:				0x10, level: '2,3',		desc: translate('Запрет записи параметров по PLC1'),
                cmd:				0x11, level: '2,3',		desc: translate('Запись параметров PLC1'),
                cmd:				0x14, level: '1,2,3',	desc: translate('Изменить параметры связи дополнительного интерфейса'),
                cmd:				0x15, level: '1,2,3',	desc: translate('Изменить параметры связи основного интерфейса'),
                Reboot: 			0x16, level: '2,3',		desc: translate('Перезапустить счетчик'),
                cmd:				0x18, level: '2,3',		desc: translate('Разрешить/запретить автоматический переход на зимнее/летнее время'),
                cmd:				0x19, level: '2,3',		desc: translate('Значения времени перехода для летнего и зимнего времени'),
                cmd:				0x1A, level: '2,3',		desc: translate('Запись серийного номера выносного дисплея'),
                cmd:				0x1B, level: '2,3',		desc: translate('Записать коэффициенты транс-формации Кн и Кт'),
                cmd:				0x1D, level: '2,3',		desc: translate('Записать тарифное расписание'),
                cmd:				0x1E, level: '2,3',		desc: translate('Записать расписание праздничных дней'),
                setPwd: 			0x1F, level: '2,3',		desc: translate('Изменить пароль'),
                cmd:				0x21, level: '2,3',		desc: translate('Инициализация регистров энергии'),
                setLocation:		0x22, level: '2,3',		desc: translate('Запись местоположения прибора'),
                cmd:				0x23, level: '2,3',		desc: translate('Запись расписания утреннего и вечернего максимумов'),
                ClearMax:			0x24, level: '2,3',		desc: translate('Сброс значений массива помесячных максимумов'),
                cmd:				0x26, level: '2,3',		desc: translate('Установка времени контроля за превышением лимита мощностии параметров автовключения реле'),
                cmd:				0x27, level: '2,3',		desc: translate('Изменение постоянной счетчика'),
                cmd:				0x2A, level: '2,3',		desc: translate('Изменение режима тарификатора'),
                setLimitAPower:		0x2C, level: '2,3',		desc: translate('Установка лимита активной мощности'),
                cmd:				0x2D, level: '2,3',		desc: translate('Включение контроля превышения лимита активной мощности'),
                cmd:				0x2E, level: '2,3',		desc: translate('Установка лимита потребленной активной энергии'),
                setLimitAEnergy:	0x2F, level: '2,3',		desc: translate('Включение контроля превышения потребленной активной энергии'),
                setImpulse: 		0x30, level: '2,3',		desc: translate('Изменение режима импульсного выхода'),
                cmd:				0x31, level: '2,3',		desc: translate('Изменение режима управления нагрузкой'),
                setTimeout: 		0x32, level: '1,2,3',	desc: translate('Изменение множителя таймаута основного интерфейса'),
                cmd:				0x33, level: '2,3',		desc: translate('Изменение режима учета технических потерь'),
                cmd:				0x34, level: '2,3',		desc: translate('Установка значений мощностей технических потерь'),
                cmd:				0x35, level: '2,3',		desc: translate('Изменение режима светодиодного индикатора и импульсного выхода R+ по виду энергии'),
                cmd:				0x36, level: '2,3',		desc: translate('Установка допустимых значений при контроле ПКЭ'),
                cmd:				0x37, level: '2,3',		desc: translate('Установка времени усреднения значений напряжения и частоты'),*/
            ]
        }
    },
    respcode: {
...
function getArraysMonthCmd1(){
    for (let month = 1; month <= 12; month++) {
        ...
        options.protocol['1'].first.push({
            code: 0x32,
            cmd:  [_month],
            desc: translate('Чтение накопленной энергии за ' + month + ' месяц'),
        ...
        for (let tariff = 0; tariff <= 4; tariff++) {
            ...
            if (month === date.getMonth() + 1){
                options.protocol['2'].poll.push({
                    code: 0x05,
                    cmd:  [_month, _tariff],
                    desc: translate('Чтение накопленной энергии за текущий месяц, ' + tariff ? 'тариф - ' + tariff :'общая'),
                    func: f.month12Energy
                });
            }
        }
