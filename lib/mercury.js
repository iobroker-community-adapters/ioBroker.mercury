    ...
    for (let month = 1; month <= 12; month++) {
        for (let tariff = 0; tariff <= 4; tariff++) {
            ...
            options.protocol['2'].first.push({
                code: 0x05,
                cmd:  [_month, _tariff],
                desc: translate('Чтение накопленной энергии за ' + month + ' месяц'), func: f.month12Energy
            });
            if (month === date.getMonth() + 1){
                options.protocol['2'].poll.push({
                    code: 0x05,
                    cmd:  [_month, _tariff],
                    desc: translate('Чтение накопленной энергии за текущий месяц, ' + tariff ? 'тариф - ' + tariff :'общая'), func: f.month12Energy
                });
            }
        }
    }
