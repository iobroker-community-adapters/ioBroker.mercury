...
const ff = {
    readSN:           function (device, index, cmd, response){
        ...
        // [0,14,31,155,47   ,0,166,182,27,  122,24]
    ...
    dateProd:         function (device, index, cmd, response){
        ...
        //{"type":"Buffer","data":[0,14,31,155,102,  37,1,18,  70,166]} length: 10 25 1 12
        setDevState(device, index, {
            name: 'info.dateProd',
    ...
    identCounter:     function (device, index, cmd, response){
        ...
        // [0,14,31,155,40,  8,3,   0,23,1,18   ,127,45]
        setDevState(device, index, {
            name: 'info.firmware',
    ...
    batteryVolt:      function (device, index, cmd, response){
        ...
        // [0,14,31,155,41,  3,5,  177,243]
        setDevState(device, index, {
            name: 'metering.battery',
            desc: 'battery',
            val:  response[5] + '.' + (response[6].toString(16) < 10 ? '0' :'') + response[6].toString(16)
        });
        return device;
    },
    totalEnergy:      function (device, index, cmd, response){
        ...
        //[0,14,31,155,39,  0,73,22,21,  0,23,36,147,   0,24,17,67,   0,0,0,0,   209,21]
        //От сброса   / 4916.15 / 1724.93 / 1811.43 / 0.00 / 8452.51 
        const t1 = parseInt(response[5].toString(16) + response[6].toString(16) + response[7].toString(16) + response[8].toString(16));
        const t2 = parseInt(response[9].toString(16) + response[10].toString(16) + response[11].toString(16) + response[12].toString(16));
        const t3 = parseInt(response[13].toString(16) + response[14].toString(16) + response[15].toString(16) + response[16].toString(16));
        const t4 = parseInt(response[17].toString(16) + response[18].toString(16) + response[19].toString(16) + response[20].toString(16));
        setDevState(device, index, {
    ...
    readUIP:          function (device, index, cmd, response){
        ...
        //[0,14,31,155,99,  36,2   ,0,21,   0,0,32,   152,112]
    ...
    voltageOff:       function (device, index, cmd, response){
        ...
        //[0,14,31,155,43,   4, 35,70,17,5, 18, 25,    238,217]
        //                   ow-hh-mm-ss-dd-mon-yy
    ...
    limitPower:       function (device, index, cmd, response){
        ...
        //[0,14,31,155,34,  153,153,  170,248]
        setDevState(device, index, {
    ...
    currentPower:     function (device, index, cmd, response){
        ...
        setDevState(device, index, {
            ...
            desc: 'currentPower',
            val:  response[5].toString(16) + '.' + (response[6].toString(16) < 10 ? '0' :'') + response[6].toString(16)
        });
        return device;
    },
    month12Energy:    function (device, index, cmd, response){
        ...
        //[0,14,31,155,50,  0,73,22,   21,0,23,   36,147,0,  24,17,67,  0,0,0,0,    207,129]
        //[0,14,31,155,50,  7,0,99,   0,255,255,   255,255,255,   255,255,255,   255,255,255,   255,127,86]
        //0,14,31,155,50,11
        ...
        let t1 = parseInt(response[5].toString(16) + response[6].toString(16) + response[7].toString(16) + response[8].toString(16)) / 100;
        let t2 = parseInt(response[9].toString(16) + response[10].toString(16) + response[11].toString(16) + response[12].toString(16)) / 100;
        let t3 = parseInt(response[13].toString(16) + response[14].toString(16) + response[15].toString(16) + response[16].toString(16)) / 100;
        let t4 = parseInt(response[17].toString(16) + response[18].toString(16) + response[19].toString(16) + response[20].toString(16)) / 100;
...
const f = {
    ...
    currentDayEnergy: function (device, index, cmd, response){
        ...
        if (response.length === 19){
            const {one, two, three, four} = getFour32LE(response, 1000);
            setDevState(device, index, {
                name: 'metering.Day.EnergyActivePlus',
                desc: 'currentDayEnergyActivePlus',
                val:  one
            });
            setDevState(device, index, {
                name: 'metering.Day.EnergyActiveMinus',
                desc: 'currentDayEnergyActiveMinus',
                val:  two
            });
            setDevState(device, index, {
                name: 'metering.Day.EnergyReactivePlus',
                desc: 'currentDayEnergyReactivePlus',
                val:  three
            });
            setDevState(device, index, {
                name: 'metering.Day.EnergyReactiveMinus',
    ...
    powerPhase:       function (device, index, cmd, response){
        ...
        if (response.length === 15){
            ...
            setDevState(device, index, {name: 'metering.MainsParameters.powerTotal', desc: 'powerTotal', val: one});
            setDevState(device, index, {name: 'metering.MainsParameters.powerPhase1', desc: 'powerPhase1', val: two});
            setDevState(device, index, {name: 'metering.MainsParameters.powerPhase2', desc: 'powerPhase2', val: three});
            setDevState(device, index, {name: 'metering.MainsParameters.powerPhase3', desc: 'powerPhase3', val: four});
    ...
    yearEnergy:       function (device, index, cmd, response){
        ...
        if (response.length === 19){
            const {one, two, three, four} = getFour32LE(response, 1000);
            setDevState(device, index, {
                name: 'metering.Year.EnergyActivePlus',
                desc: 'currentYearEnergyActivePlus',
                val:  one
            });
            setDevState(device, index, {
                name: 'metering.Year.EnergyActiveMinus',
                desc: 'currentYearEnergyActiveMinus',
                val:  two
            });
            setDevState(device, index, {
                name: 'metering.Year.EnergyReactivePlus',
                desc: 'currentYearEnergyReactivePlus',
                val:  three
            });
            setDevState(device, index, {
                name: 'metering.Year.EnergyReactiveMinus',
            ...
            poll:     [
                {
                    code: 0x27,
                    cmd:  [],
                    desc: 'Чтение содержимого тарифных аккумуляторов активной энергии',
                    func: ff.totalEnergy
                },
                ...
                //{code: 0x37, cmd: [0x01], desc: 'Чтение получасовых мощностей или суточных срезов', func: ff.readDay}, //ADDR-CMD-N48-dd-mon-yy-CRC
