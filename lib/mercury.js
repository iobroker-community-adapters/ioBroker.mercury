...
=======
function setDevState(device, param, val){
    let arr = param.name.split('.');
    const obj = {val:  val, desc: param.desc ? param.desc :''};
    if(arr.length > 2) return;
    for (let i = 0; i < arr.length; i++) {
        if (i === 0 && !device[param.index][arr[0]]){
    		device[param.index][arr[0]] = {};
    		if(arr.length - 1 === i){
    			device[param.index][arr[0]] = obj;
    		}
	    }
	    if (i === 1 && !device[param.index][arr[0]][arr[1]]){
		   device[param.index][arr[0]][arr[1]] = {};
		   if(arr.length - 1 === i){
    			device[param.index][arr[0]][arr[1]] = obj;
    		}
		}
		if (i === 2 && !device[param.index][arr[0]][arr[1]][arr[2]]){
		   device[param.index][arr[0]][arr[1]][arr[2]] = {};
		   if(arr.length - 1 === i){
    			device[param.index][arr[0]][arr[1]][arr[2]] = obj;
    		}
		}
    }
    return device;
...
var f = {
    readSnAndDate:    function (device, index, cmd, response){
        console.log('readSnAndDate - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' lenght: ' + response.length);
        setDevState(device, {index: index, name:  'info.sn', desc:  'Serial number'}, parseInt(response.slice(1, 5).join('')));
		setDevState(device, {index: index, name:  'info.dateProd', desc:  'Production date'}, (parseInt(response[5]) < 10 ? '0' :'') + response[5] + '.' + (parseInt(response[6]) < 10 ? '0' :'') + response[6] + '.' + '20' + response[7]);
>>>>>>> origin/master
        return device;
    },
...
=======
        setDevState(device, {index: index, name:  'info.firmware', desc:  'Software version'}, response.slice(1, 4).join('.') );
>>>>>>> origin/master
        return device;
    },
...
=======
        //device[index].info.crcDevice.val = response.slice(1, 3).toString('hex').toUpperCase();
        setDevState(device, {index: index, name:  'info.crcDevice', desc:  'CRC16 software'}, response.slice(1, 3).toString('hex').toUpperCase() );
>>>>>>> origin/master
        return device;
    },
...
=======
        //device[index].info.classEnergyA.val = getVariant(1, 4, (response[1] >> 6) & 3);
        setDevState(device, {index: index, name:  'info.classEnergyA', desc:  'Energy Class A+'}, getVariant(1, 4, (response[1] >> 6) & 3) );
        //device[index].info.classEnergyR.val = getVariant(1, 3, (response[1] >> 4) & 3);
        setDevState(device, {index: index, name:  'info.classEnergyR', desc:  'Energy Class R+'}, getVariant(1, 3, (response[1] >> 4) & 3) );
        //device[index].info.nomU.val = getVariant(1, 2, (response[1] >> 2) & 3);
        setDevState(device, {index: index, name:  'info.nomU', desc:  'Rated voltage'}, getVariant(1, 2, (response[1] >> 2) & 3) );
        //device[index].info.nomI.val = getVariant(1, 1, (response[1] >> 0) & 3);
        setDevState(device, {index: index, name:  'info.nomI', desc:  'Rated current'}, getVariant(1, 1, (response[1] >> 0) & 3) );
        //2
        //device[index].info.dest.val = getVariant(2, 5, (response[2] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.dest', desc:  'Number of destinations'}, getVariant(2, 5, (response[2] >> 7) & 1) );
        //device[index].info.temperature.val = getVariant(2, 4, (response[2] >> 6) & 1);
        setDevState(device, {index: index, name:  'info.temperature', desc:  'Temperature Range'}, getVariant(2, 4, (response[2] >> 6) & 1) );
        //device[index].info.mediumPowerProfile.val = getVariant(2, 3, (response[2] >> 5) & 1);
        setDevState(device, {index: index, name:  'info.mediumPowerProfile', desc:  'Medium Capacity Profile capacities'}, getVariant(2, 3, (response[2] >> 5) & 1) );
        //device[index].info.numPhases.val = getVariant(2, 2, (response[2] >> 4) & 1);
        setDevState(device, {index: index, name:  'info.numPhases', desc:  'Number of phases'}, getVariant(2, 2, (response[2] >> 4) & 1) );
        //device[index].info.counterConst.val = getVariant(2, 1, (response[2] >> 0) & 15);
        setDevState(device, {index: index, name:  'info.counterConst', desc:  'Counter constant'}, getVariant(2, 1, (response[2] >> 0) & 15) );
        //3
        //device[index].info.SummPhases.val = getVariant(3, 4, (response[3] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.SummPhases', desc:  'Phase Summation'}, getVariant(3, 4, (response[3] >> 7) & 1) );
        //device[index].info.Tarifficator.val = getVariant(3, 3, (response[3] >> 6) & 1);
        setDevState(device, {index: index, name:  'info.Tarifficator', desc:  'Tariff'}, getVariant(3, 3, (response[3] >> 6) & 1) );
        //device[index].info.typeCount.val = getVariant(3, 2, (response[3] >> 4) & 3);
        setDevState(device, {index: index, name:  'info.typeCount', desc:  'Counter type'}, getVariant(3, 2, (response[3] >> 4) & 3) );
        //device[index].info.executionOption.val = parseInt((response[3] >> 0) & 15);
        setDevState(device, {index: index, name:  'info.executionOption', desc:  'Execution option'}, parseInt((response[3] >> 0) & 15) );
        //4
        //device[index].info.memSize.val = getVariant(4, 7, (response[4] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.memSize', desc:  'Memory'}, getVariant(4, 7, (response[4] >> 7) & 1) );
        //device[index].info.intPlm.val = getVariant(4, 6, (response[4] >> 6) & 1);
        setDevState(device, {index: index, name:  'info.intPlm', desc:  'Built-in PLM Modem'}, getVariant(4, 6, (response[4] >> 6) & 1) );
        //device[index].info.intGsm.val = getVariant(4, 5, (response[4] >> 5) & 1);
        setDevState(device, {index: index, name:  'info.intGsm', desc:  'Built-in GSM Modem'}, getVariant(4, 5, (response[4] >> 5) & 1) );
        //device[index].info.ir.val = getVariant(4, 4, (response[4] >> 4) & 1);
        setDevState(device, {index: index, name:  'info.ir', desc:  'Optical port'}, getVariant(4, 4, (response[4] >> 4) & 1) );
        //device[index].info.interface.val = getVariant(4, 3, (response[4] >> 2) & 3);
        setDevState(device, {index: index, name:  'info.interface', desc:  'Interface type'}, getVariant(4, 3, (response[4] >> 2) & 3) );
        //device[index].info.extPower.val = getVariant(4, 2, (response[4] >> 1) & 1);
        setDevState(device, {index: index, name:  'info.extPower', desc:  'External power'}, getVariant(4, 2, (response[4] >> 1) & 1) );
        //device[index].info.elecSealTopCover.val = getVariant(4, 1, (response[4] >> 0) & 1);
        setDevState(device, {index: index, name:  'info.elecSealTopCover', desc:  'Top Seal Electronic Seal'}, getVariant(4, 1, (response[4] >> 0) & 1) );
        //5
        //device[index].info.internalRelay.val = getVariant(5, 8, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.internalRelay', desc:  'Built-in load shedding relay'}, getVariant(5, 8, (response[5] >> 7) & 1) );
        //device[index].info.backlightDisp.val = getVariant(5, 7, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.backlightDisp', desc:  'LCD backlight'}, getVariant(5, 7, (response[5] >> 7) & 1) );
        //device[index].info.tariffMeteringOfMaxPower.val = getVariant(6, 5, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.tariffMeteringOfMaxPower', desc:  'Tariff metering of maximum power'}, getVariant(6, 5, (response[5] >> 7) & 1) );
        //device[index].info.elecSealCover.val = getVariant(5, 5, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.elecSealCover', desc:  'Electronic seal of protective cover'}, getVariant(5, 5, (response[5] >> 7) & 1) );
        //device[index].info.interface2.val = getVariant(5, 4, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.interface2', desc:  'Interface2'}, getVariant(5, 4, (response[5] >> 7) & 1) );
        //device[index].info.intPower.val = getVariant(5, 3, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.intPower', desc:  'Built-in Power Interface1'}, getVariant(5, 3, (response[5] >> 7) & 1) );
        //device[index].info.controlPke.val = getVariant(5, 2, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.controlPke', desc:  'PCE control'}, getVariant(5, 2, (response[5] >> 7) & 1) );
        //device[index].info.phaseMeteringEnergyA.val = getVariant(5, 1, (response[5] >> 7) & 1);
        setDevState(device, {index: index, name:  'info.phaseMeteringEnergyA', desc:  'Phase energy metering A+'}, getVariant(5, 1, (response[5] >> 7) & 1) );
        //6
        //device[index].info.internalPlc2.val = getVariant(6, 5, (response[6] >> 4) & 1);
        setDevState(device, {index: index, name:  'info.internalPlc2', desc:  'Integrated PLC2 Modem'}, getVariant(6, 5, (response[6] >> 4) & 1) );
        //device[index].info.profile2.val = getVariant(6, 4, (response[6] >> 3) & 1);
        setDevState(device, {index: index, name:  'info.profile2', desc:  'Profile2'}, getVariant(6, 4, (response[6] >> 3) & 1) );
        //device[index].info.electronicSealModuleBay.val = getVariant(6, 3, (response[6] >> 2) & 1);
        setDevState(device, {index: index, name:  'info.electronicSealModuleBay', desc:  'Modular compartment electronic seal'}, getVariant(6, 3, (response[6] >> 2) & 1) );
        //device[index].info.externalVoltageTariffSwitch.val = getVariant(6, 2, (response[6] >> 1) & 1);
        setDevState(device, {index: index, name:  'info.externalVoltageTariffSwitch', desc:  'Tariff switching by external voltage'}, getVariant(6, 2, (response[6] >> 1) & 1) );
...
=======
        //device[index].info.location.val = response.toString('ascii', 1, 5);
        setDevState(device, {index: index, name:  'info.location', desc:  'Location'}, response.toString('ascii', 1, 5) );
>>>>>>> origin/master
        return device;
    },
...
=======
        //device[index].info.voltageTransformationRatio.val = response.readInt16BE(1);
        setDevState(device, {index: index, name:  'info.voltageTransformationRatio', desc:  'Voltage transformation ratio'}, response.readInt16BE(1) );
        //device[index].info.currentTransformationRatio.val = response.readInt16BE(3);
        setDevState(device, {index: index, name:  'info.currentTransformationRatio', desc:  'Current transformer ratio'}, response.readInt16BE(3) );
>>>>>>> origin/master
        return device;
    },
    totalEnergy:      function (device, index, cmd, response){
        ...
        if (response.length === 19){
            const {one, two, three, four} = getFour32LE(response);
            setDevState(device, index, {
    ...
    month12Energy:    function (device, index, cmd, response){
        ...
        if (response.length === 19){
            ...
            const {one, two, three, four} = getFour32LE(response);
            let ttl = (one + two + three + four) / 1000;
            let trf;
...
const variant = {
    ...
    2: {
        1: {0: '5000 imp/1kW', 1: '25000 imp/1kW', 2: '1250 imp/1kW', 3: '500 imp/1kW', 4: '1000 imp/1kW', 5: '250 imp/1kW'},
        2: {0: '3', 1: '1'},
        3: {0: 'no', 1: 'yes'},
        4: {0: '-20', 1: '-40'},
        5: {0: '2', 1: '1'}
...
function getThree32BE(response, divider){
    ...
    return {
        one:   (Buffer.from([0, 0, response[3], response[2]]).readInt32BE(0)) / divider,
        two:   (Buffer.from([0, 0, response[6], response[5]]).readInt32BE(0)) / divider,
        three: (Buffer.from([0, 0, response[9], response[8]]).readInt32BE(0)) / divider
    };
}
...
function getFour32BE(response, divider){
    ...
    return {
        ...
        two:   (Buffer.from([0, 0, response[6], response[5]]).readInt32BE(0)) / divider,
        three: (Buffer.from([0, 0, response[9], response[8]]).readInt32BE(0)) / divider,
        four:  (Buffer.from([0, 0, response[12], response[11]]).readInt32BE(0)) / divider
    };
}
...
function getFour32LE(response, divider){
    if (!divider) divider = 1;
    return {
        one:   (Buffer.concat([response.slice(3, 5), response.slice(1, 3)]).readInt32LE(0)) / divider,
        two:   (Buffer.concat([response.slice(7, 9), response.slice(5, 7)]).readInt32LE(0)) / divider,
        three: (Buffer.concat([response.slice(11, 13), response.slice(9, 11)]).readInt32LE(0)) / divider,
        four:  (Buffer.concat([response.slice(15, 17), response.slice(13, 15)]).readInt32LE(0)) / divider
    };
}
