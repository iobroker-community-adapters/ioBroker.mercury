...
const translate = function (w){ // PLEASE HELP WITH THE TRANSLATION
    const word = {
        'DateTime': {ru: 'Дата/Время'},
        'Serial number': {ru: 'Серийный номер'},
        'Production date': {ru: 'Дата изготовления'},
        'Software version': {ru: 'Версия ПО'},
    };
    return word[w]['ru'] ? word[w]['ru'] : w;
};
...
const ff = {
    readSN:           function (device, index, cmd, response){
        console.log('readSN - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    dateProd:         function (device, index, cmd, response){
        console.log('dateProd - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'info.dateProd',
    ...
    identCounter:     function (device, index, cmd, response){
        console.log('identCounter - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'info.firmware',
            ...
            setDevState(device, index, {
                ...
                desc: 'firmware date',  //Дата версии прошивки
    ...
    optronFunc:       function (device, index, cmd, response){
        console.log('optronFunc - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            const vars = {
                0: '5000 imp/kW',
                1: '10000 imp/kW',
    ...
    variantExecution: function (device, index, cmd, response){
        console.log('variantExecution - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'info.twoCurrentSensors',
                desc: 'twoCurrentSensors',
                val:  response[5] > 0
            });
            setDevState(device, index, {
    ...
    batteryVolt:      function (device, index, cmd, response){
        console.log('batteryVolt - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
    ...
    totalEnergy:      function (device, index, cmd, response){
        console.log('totalEnergy - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            const {t1, t2, t3, t4} = parseEnergy(response, 100);
            setDevState(device, index, {
                name: 'metering.Total.T1',
                desc: 'oneTarrifEnergyActive',
                unit: 'kWh',
                val:  t1
            });
            setDevState(device, index, {
                name: 'metering.Total.T2',
                desc: 'twoTarrifEnergyActive',
                unit: 'kWh',
                val:  t2
            });
            setDevState(device, index, {
                name: 'metering.Total.T3',
                desc: 'threeTarrifEnergyActive',
                unit: 'kWh',
                val:  t3
            });
            setDevState(device, index, {
    ...
    readUIP:          function (device, index, cmd, response){
        console.log('readUIP - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            const u = parseInt(response[5].toString(16) + response[6].toString(16));
            const i = parseInt(response[7].toString(16) + response[8].toString(16));
            const p = parseInt(response[9].toString(16) + response[10].toString(16) + response[11].toString(16));
            setDevState(device, index, {
    ...
    voltageOff:       function (device, index, cmd, response){
        console.log('voltageOff - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    voltageOn:        function (device, index, cmd, response){
        console.log('voltageOn - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            let date = parseDate(response[9], response[10], response[11]);
            date += ' ' + parseTime(response[6], response[7], response[8]);
            setDevState(device, index, {
    ...
    readDateTime:     function (device, index, cmd, response){
        console.log('readDateTime - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            let date = parseDate(response[9], response[10], response[11]);
            date += ' ' + parseTime(response[6], response[7], response[8]);
            setDevState(device, index, {
    ...
    limitPower:       function (device, index, cmd, response){
        console.log('limitPower - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
    ...
    limitEnergyMonth: function (device, index, cmd, response){
        console.log('limitEnergyMonth - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'metering.limitEnergyMonth',
                desc: 'limitEnergyMonth',
                unit: 'kWh',
    ...
    currentPower:     function (device, index, cmd, response){
        console.log('currentPower - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
    ...
    month12Energy:    function (device, index, cmd, response){
        console.log('month12Energy - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        console.log('month12Energy - cmd: ' + JSON.stringify(cmd));
        try {
            ...
            if (month !== 16 && device[index].metering.EnergyMonth[month] === undefined){
                device[index].metering.EnergyMonth[month] = {
                    T1:    {val: null, unit: 'kWh', desc: 'Энергия по тарифу 1'},
                    T2:    {val: null, unit: 'kWh', desc: 'Энергия по тарифу 2'},
                    T3:    {val: null, unit: 'kWh', desc: 'Энергия по тарифу 3'},
                    T4:    {val: null, unit: 'kWh', desc: 'Энергия по тарифу 4'},
                    Total: {val: null, unit: 'kWh', desc: 'Энергия Итого'}
                };
            }
    ...
    readMaxU:         function (device, index, cmd, response){
        console.log('readMaxU - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    runningTime:      function (device, index, cmd, response){
        console.log('runningTime - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            //ADDR-CMD-DATA24 -CRC
            /*setDevState(device, index, {
...
const f = {
    readSnAndDate:    function (device, index, cmd, response){
        console.log('readSnAndDate - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'info.sn',
    ...
    firmwareVersion:  function (device, index, cmd, response){
        console.log('firmwareVersion - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'info.firmware',
    ...
    crcDevice:        function (device, index, cmd, response){
        console.log('crcDevice - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
                name: 'info.crcDevice',
                desc: 'CRC16 software',
    ...
    readLocation:     function (device, index, cmd, response){
        console.log('readLocation - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    transformRatio:   function (device, index, cmd, response){
        console.log('transformRatio - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            setDevState(device, index, {
    ...
    currentDayEnergy: function (device, index, cmd, response){
        console.log('currentDayEnergy - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
                const {one, two, three, four} = getFour32LE(response, 1000);
                setDevState(device, index, {
                    name: 'metering.Day.TotalEnergyActivePlus',
                    desc: 'currentDayTotalEnergyActivePlus',
                    unit: 'kWh',
                    val:  one
                });
                setDevState(device, index, {
                    name: 'metering.Day.TotalEnergyActiveMinus',
                    desc: 'currentDayTotalEnergyActiveMinus',
                    unit: 'kWh',
                    val:  two
                });
                setDevState(device, index, {
                    name: 'metering.Day.TotalEnergyReactivePlus',
                    desc: 'currentDayTotalEnergyReactivePlus',
                    unit: 'kvar',
                    val:  three
                });
                setDevState(device, index, {
                    name: 'metering.Day.TotalEnergyReactiveMinus',
                    desc: 'currentDayTotalEnergyReactiveMinus',
                    unit: 'kvar',
                    val:  four
                });
                /*setDevState(device, index, {
    ...
    oneTarrif:        function (device, index, cmd, response){
        console.log('oneTarrif - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
                const {one, two, three, four} = getFour32LE(response, 1000);
                setDevState(device, index, {
    ...
    twoTarrif:        function (device, index, cmd, response){
        console.log('twoTarrif - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
                const {one, two, three, four} = getFour32LE(response, 1000);
                setDevState(device, index, {
    ...
    threeTarrif:      function (device, index, cmd, response){
        console.log('threeTarrif - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
                const {one, two, three, four} = getFour32LE(response, 1000);
                setDevState(device, index, {
    ...
    fourTarrif:       function (device, index, cmd, response){
        console.log('fourTarrif - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
                const {one, two, three, four} = getFour32LE(response, 1000);
                setDevState(device, index, {
    ...
    powerPhase:       function (device, index, cmd, response){
        console.log('powerPhase - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 15){
                const {one, two, three, four} = getFour32BE(response, 100);
                setDevState(device, index, {name: 'metering.MainsParameters.powerTotal', unit: 'W', desc: 'powerTotal', val: one});
                setDevState(device, index, {name: 'metering.MainsParameters.powerPhase1', unit: 'W', desc: 'powerPhase1', val: two});
                setDevState(device, index, {name: 'metering.MainsParameters.powerPhase2', unit: 'W', desc: 'powerPhase2', val: three});
                setDevState(device, index, {name: 'metering.MainsParameters.powerPhase3', unit: 'W', desc: 'powerPhase3', val: four});
            }
        } catch (e) {
            console.log(e);
        }
    ...
    voltagePhase:     function (device, index, cmd, response){
        console.log('voltagePhase - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    currentPhase:     function (device, index, cmd, response){
        console.log('currentPhase - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 12){
    ...
    cosf:             function (device, index, cmd, response){
        console.log('cosf - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 15){
                const {one, two, three, four} = getFour32BE(response, 1000);
                console.log('cosf - t1: ' + one + ' t2: ' + two + ' t3: ' + three + ' t4: ' + four);
                setDevState(device, index, {name: 'metering.MainsParameters.cosfTotal', desc: 'cosfTotal', val: one});
                setDevState(device, index, {name: 'metering.MainsParameters.cosfPhase1', desc: 'cosfPhase1', val: two});
                setDevState(device, index, {name: 'metering.MainsParameters.cosfPhase2', desc: 'cosfPhase2', val: three});
                setDevState(device, index, {name: 'metering.MainsParameters.cosfPhase3', desc: 'cosfPhase3', val: four});
            }
        } catch (e) {
            console.log(e);
        }
    ...
    yearEnergy:       function (device, index, cmd, response){
        console.log('yearEnergy - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
                const {one, two, three, four} = getFour32LE(response, 1000);
                setDevState(device, index, {
                    name: 'metering.Year.TotalEnergyActivePlus',
                    desc: 'currentYearTotalEnergyActivePlus',
                    unit: 'kWh',
                    val:  one
                });
                setDevState(device, index, {
                    name: 'metering.Year.TotalEnergyActiveMinus',
                    desc: 'currentYearTotalEnergyActiveMinus',
                    unit: 'kWh',
                    val:  two
                });
                setDevState(device, index, {
                    name: 'metering.Year.TotalEnergyReactivePlus',
                    desc: 'currentYearTotalEnergyReactivePlus',
                    unit: 'kvar',
                    val:  three
                });
                setDevState(device, index, {
                    name: 'metering.Year.TotalEnergyReactiveMinus',
                    desc: 'currentYearTotalEnergyReactiveMinus',
                    unit: 'kvar',
                    val:  four
                });
                /*setDevState(device, index, {
    ...
    month12Energy:    function (device, index, cmd, response){
        console.log('monthEnergy - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            if (response.length === 19){
    ...
    readCurrentTime:     function (device, index, cmd, response){
        console.log('readCurrentTime - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
    ...
    statusByte:             function (device, index, cmd, response){
        console.log('statusByte - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
...
const statusByte = {
    6: [
        {code: 'Е-01', desc: 'Напряжение батареи менее 2,2 В'}, 
        {code: 'Е-02', desc: 'Нарушено функционирование памяти №2'},
        {code: 'Е-03', desc: 'Нарушено функционирование UART1'},
        {code: 'Е-04', desc: 'Нарушено функционирование ADS'},
        {code: 'Е-05', desc: 'Ошибка обмена с памятью №1'},
        {code: 'Е-06', desc: 'Нарушено функционирование RTC'},
        {code: 'Е-07', desc: 'Нарушено функционирование памяти №3'},
        {code: 'Е-08', desc: ''}
    ],
    5: [
        {code: 'Е-09', desc: 'Ошибка КС программы'},
        {code: 'Е-10', desc: 'Ошибка КС массива калибровочных коэфф. во Flash MSP430'},
        {code: 'Е-11', desc: 'Ошибка КС массива регистров накопленной энергии'},
        {code: 'Е-12', desc: 'Ошибка КС адреса прибора'},
        {code: 'Е-13', desc: 'Ошибка КС серийного номера'},
        {code: 'Е-14', desc: 'Ошибка КС пароля'},
        {code: 'Е-15', desc: 'Ошибка КС массива варианта исполнения счетчика'},
        {code: 'Е-16', desc: 'Ошибка КС байта тарификатора'}
    ],
    4: [
        {code: 'Е-17', desc: 'Ошибка КС байта управления нагрузкой'},
        {code: 'Е-18', desc: 'Ошибка КС лимита мощности'},
        {code: 'Е-19', desc: 'Ошибка КС лимита энергии'},
        {code: 'Е-20', desc: 'Ошибка КС байта параметров UART'},
        {code: 'Е-21', desc: 'Ошибка КС параметров индикации(по тарифам)'},
        {code: 'Е-22', desc: 'Ошибка КС параметров индикации (по периодам)'},
        {code: 'Е-23', desc: 'Ошибка КС множителя тайм-аута'},
        {code: 'Е-24', desc: 'Ошибка КС байта программируемых флагов'}
    ],
    3: [
        {code: 'Е-25', desc: 'Ошибка КС массива праздничных дней'},
        {code: 'Е-26', desc: 'Ошибка КС массива тарифного расписания'},
        {code: 'Е-27', desc: 'Ошибка КС массива таймера'},
        {code: 'Е-28', desc: 'Ошибка КС массива сезонных переходов'},
        {code: 'Е-29', desc: 'Ошибка КС массива местоположения прибора'},
        {code: 'Е-30', desc: 'Ошибка КС массива коэффициентов трансформации'},
        {code: 'Е-31', desc: 'Ошибка КС массива регистров накопления по периодам времени'},
        {code: 'Е-32', desc: 'Ошибка КС параметров среза'}
    ],
    2: [
        {code: 'Е-33', desc: 'Ошибка КС регистров среза'},
        {code: 'Е-34', desc: 'Ошибка КС указателей журнала событий'},
        {code: 'Е-35', desc: 'Ошибка КС записи журнала событий'},
        {code: 'Е-36', desc: 'Ошибка КС регистра учета технических потерь'},
        {code: 'Е-37', desc: 'Ошибка КС мощностей технических потерь'},
        {code: 'Е-38', desc: 'Ошибка КС массива регистров накопленной энергии потерь'},
        {code: 'Е-39', desc: 'Ошибка КС регистров энергии пофазного учета'},
        {code: 'Е-40', desc: 'Флаг поступления широковещательного сообщения'}
    ],
    1: [
        {code: 'Е-41', desc: 'Ошибка КС указателей журнала ПКЭ'},
        {code: 'Е-42', desc: 'Ошибка КС записи журнала ПКЭ'},
        {code: 'Е-43', desc: ''},
        {code: 'Е-44', desc: ''},
        {code: 'Е-45', desc: ''},
        {code: 'Е-46', desc: ''},
        {code: 'Е-47', desc: 'Флаг выполнения процедуры коррекции времени'},
        {code: 'Е-48', desc: 'Напряжение батареи менее 2,65 В'}
    ]
};
    ...
    protocol: {
        ...
        2: {
            ...
            read:     [
                ...
                {
                    cmd:  0x0C, desc: 'Чтение расписания утренних и вечерних максимумов мощности',
