...
const ff = {
    ...
    readFreq:         function (device, index, cmd, response){
        ...
        try {
            ...
            setDevState(device, index, {
                name: 'metering.MainsParameters.freq',
                desc: translate('freq'),
                unit: 'Hz',
                val:  hz > 999 ? hz / 10 :hz
            });
    ...
    readParams:       function (device, index, cmd, response){
        //event.emit('debug','readUIP - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        event.emit('debug', 'readParams - index: ' + JSON.stringify(index) + ' response: ' + JSON.stringify(response) + ' length: ' + response.length);
        try {
            const hz = parseInt(response[5].toString(16) + zeroConcat(response[6].toString(16)));
            /*setDevState(device, index, {
                name: 'metering.MainsParameters.freq',
    ...
    protocol: {
        1: {
            ...
            fastpoll: [
                ...
                {code: 0x86, cmd: [0x00], desc: 'Чтение параметра 0', func: ff.readParams},
                {code: 0x86, cmd: [0x01], desc: 'Чтение параметра 1', func: ff.readParams}
